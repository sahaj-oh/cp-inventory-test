    ...
    <div>
      <div className="page-head">
        ...
        <div className="ph-sub muted">
          {loading ? <Loading /> : (counts.Total != null ? `${counts.Total} total submissions` : 'Summary')}
        </div>
      </div>
      ...
      {!isViewer && (
        <Link
          ...
          style={{
            '--tc': '#2563eb',
            display: 'flex', alignItems: 'center', gap: 16, textDecoration: 'none', marginBottom: 18,
          }}
        >
      ...
      <div className="home-quad">
        {GROUPS.map((g) => (
          <div key={g.title} className="quad-card" style={{ '--qc': g.accent }}>
